...
TEST(Environment, Constructor) {
    ...
    nlohmann::json j;
    configStream >> j;
    EXPECT_NO_THROW(controller::Environment{j.at("game").get<controller::GameConfig>()});
}
...
TEST(Environment, SingleUpdate) {
    ...
    nlohmann::json j;
    configStream >> j;
    controller::Environment environment{j.at("game").get<controller::GameConfig>()};
...
TEST(Environment, PlayerUp) {
    ...
    EXPECT_NO_FATAL_FAILURE(environment.playerUp(0));
    EXPECT_DEATH(environment.playerUp(-2), "");
...
TEST(Environment, UpdateFail) {
    ...
    EXPECT_DEATH(environment.update(-2), "");
    EXPECT_NO_FATAL_FAILURE(environment.update(1));
    EXPECT_NO_FATAL_FAILURE(environment.update(0));
}
